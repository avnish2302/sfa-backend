...
export const punchOut = async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO punch_outs (punch_in_id, user_id, end_odometer_reading, shops_visited, shops_pending) VALUES (?, ?, ?, ?, ?)`, [punchInId, userId, end_odometer_reading, shops_visited, shops_pending])

    // Update punch_ins to inactive
    await pool.query("UPDATE punch_ins SET is_active = FALSE WHERE id = ?", [punchInId])
...
export const getPunchSummary = async (req, res) => {
  try {
    ...
    const [activePunch] = await pool.query(
      "SELECT id, own_vehicle FROM punch_ins WHERE user_id = ? AND is_active = TRUE", [userId])

    if (activePunch.length === 0) return res.status(400).json({ message: "No active punch" });
    
    const punchInId = activePunch[0].id
    const ownVehicle = activePunch[0].own_vehicle

    const [[shops]] = await pool.query("SELECT COUNT(*) as count FROM checkins WHERE punch_in_id = ?", [punchInId])

    const [[inventoryEntered]] = await pool.query(
      `SELECT SUM(
    COALESCE(cases_cold,0) + COALESCE(cases_warm,0) + COALESCE(bottles_cold,0) + COALESCE(bottles_warm,0)) AS count FROM own_inventory
    WHERE checkin_id IN (SELECT id FROM checkins WHERE punch_in_id = ?)`, [punchInId],
    )

    const [[cash]] = await pool.query(
      `SELECT IFNULL(SUM(amount),0) as total FROM collection WHERE checkin_id IN (SELECT id FROM checkins WHERE punch_in_id = ?)`, [punchInId])
